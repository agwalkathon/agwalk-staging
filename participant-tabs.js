...
(function() {
  ...
  function openStatsDetail(type) {
    ...
    var summaryEl = document.getElementById('stats-detail-summary');
    var canvas = document.getElementById('stats-detail-chart');
    if (!canvas) return;
    ...
    // Filter valid activities (non-flagged)
    var validActs = (CURRENT_ACTS || []).filter(function(a) { return !a.is_flagged; });
    ...
    if (type === 'activities') {
      chartTitle = 'Activity Type Breakdown';
      chartType = 'doughnut';
      summaryText = 'Visual distribution of your workouts by type.';

      var counts = {};
      validActs.forEach(function(a) {
        var t = a.sport_type || 'Walk';
      ...
      chartTitle = 'Daily Distance History';
      chartType = 'bar';
      summaryText = 'Kilometers logged per day over the last 14 days.';
      ...
      validActs.forEach(function(a) {
        var d = getActDate(a);
        if (d && dateKm[d] !== undefined) {
          dateKm[d] += (a.distance_meters || 0) / 1000;
        }
      });

      labels = Object.keys(dateKm).map(function(dStr) {
      ...
      chartDataSets = [{
        label: 'Distance (km)',
        data: data,
        backgroundColor: 'rgba(232, 98, 42, 0.85)',
        hoverBackgroundColor: '#FC6100',
        borderRadius: 4,
        borderWidth: 0
      }];
    } 
    else if (type === 'time') {
      chartTitle = 'Active Time Progression';
      chartType = 'line';
      summaryText = 'Daily moving duration (minutes) over the last 14 days.';
      ...
      validActs.forEach(function(a) {
        var d = getActDate(a);
        if (d && dateSec[d] !== undefined) {
          dateSec[d] += (a.moving_time_seconds || 0);
        }
      });

      labels = Object.keys(dateSec).map(function(dStr) {
        var parts = dStr.split('-');
        return parts[2] + '/' + parts[1]; // DD/MM format
      });
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            ...
            borderWidth: 1,
            padding: 10,
            displayColors: (type === 'activities'),
            callbacks: {
              label: function(context) {
                var val = context.raw;
                if (type === 'activities') {
                  return ' ' + context.label + ': ' + val + ' acts';
                } else if (type === 'distance') {
  ...
  window.closeStatsDetail = closeStatsDetail;
